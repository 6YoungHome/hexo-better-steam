...
function generatePage(globalConfig, root, config = {}) {
    let games = [];
    // source_dir = __dirname.replace('\\node_modules\\hexo-steam-games\\lib', '')
    source_dir = __dirname.split("\\").slice(0,-3).join("/"); //获取blog根目录

    if (!fs.existsSync(path.resolve(source_dir, `source/_data/steam.json`))) {
        log.info(`Can't find steam game data, please use 'hexo steam -u' command to get data`);
    } else {
        games = JSON.parse(fs.readFileSync(path.resolve(source_dir, `source/_data/steam.json`))).slice(0, config.length || globalConfig.length || 1000);
...
module.exports = async function (locals) {
    ...
    // }
    if (config.steam.steamId || config.steam.steamInfos) {
        return generatePage(config, root);
    }
